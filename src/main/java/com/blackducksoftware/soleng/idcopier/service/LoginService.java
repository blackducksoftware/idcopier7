...
public class LoginService {
	...
	public LoginService(ProtexServer protexServer) {
		try {
			...
			protexProxy = new ProtexServerProxy(protexServer.getServerName(), protexServer.getUserName(),
					protexServer.getPassword());
	...
	public String getProjectCodeTree(String projectID) {
		...
		try {
			...
			List<CodeTreeNode> nodes = protexProxy.getCodeTreeApi().getCodeTreeNodes(projectID, "/", ctrRequest);
