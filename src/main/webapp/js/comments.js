		...
		.ready(
				function() {
					...
					var populateWidgets = (function() {
						console
								.log("Populating project pulldows for the comments copying");
						$
								.each(
										locations,
										function(index, locationValue) {
											console
													.log("Processing Comment location: "
															+ locationValue);
											// Variables used for all processes
											// This is the div id of the
											// pulldown
											var serverSelectorDiv = ".select"
													+ locationValue
													+ "CommentServer";
											// This is the dev id of the project
											// pulldown
											var projectSelectorDiv = ".select"
													+ locationValue
													+ "CommentProject";
											// This is the default non-value
											// message of the selector
											var messageServer = "Select "
													+ locationValue
													+ " Comment Server";
											/**
											 * Populate server pulldown
											 */
											processJSON(servers, locationValue,
													"Servers")
													.done(
															function(data) {
																// Grab the
																// cookie, if
																// there is one
																var cachedServerName = $
																		.cookie(serverSelectorDiv);
																$(
																		serverSelectorDiv)
																		.empty();
																$(
															...
															function() {
																var serverName = $(
																		...
																		.children(
																				":selected")
																		.text();
																var path = "reloginServer/"
																		+ source
																		+ "/?server-name="
																		+ serverName;
																console
																		.log("Sending  relogin path: "
																				+ path);
																processJSON(
																		path,
																		locationValue,
													...
													.change(
															function() {
																...
																if (locationValue === source) {
																	getRefreshStatusForSourceProject(
																			serverName,
																			projectId);
																} else {
																	getRefreshStatusForTargetProject(
																			serverName,
																			projectId);
																}
															});

											var dataSet = [ [ 'true', 'Niles',
													'1.0', 'This is my comment' ] ];
					...
					$("#copyCommentsButton")
							.on(
									...
									function() {
										console
												.log("Submitting comments copy...");
										// Copy required values
										var sourceServer = $(
												'.selectSourceCommentServer')
												.children(":selected").text();
										var targetServer = $(
												'.selectTargetCommentServer')
												.children(":selected").text();
										var sourceProjectId = $(
												'.selectSourceCommentProject')
												.children(":selected").attr(
														"id");
										var targetProjectId = $(
												'.selectTargetCommentProject')
												.children(":selected").attr(
														"id");
										var components = $(
										...
										$
												.ajax({
													...
													error : function(msg) {
														...
														displayNotificationMessage(
																error,
																'Failed to copy identifications',
																msg, noisy);
													}
												});
									});
					$("#displayBomButton")
							.on(
									...
									function() {
										...
										console
												.log("Displaying the Bill of Materials for "
														+ sourceProjectName
														+ " ["
														+ sourceProjectId
														+ "] "
...
function populateProjectPerServer(serverSelectorDiv, locationValue) {
	...
	if (serverName !== null && serverName.length > 0) {
		processJSON(projectPath, locationValue, "Getting Projects").done(
				function(data) {
...
function setProjects(sender, message, data) {
	...
	if (data === null) {
		displayNotificationMessage(error, "Project Display Error",
				"No projects returned from server!", noisy);
		return false;
	}
	$.each(data, function(index, value) {
		// TODO: Should this actually be here?
		if (cachedProjectId === value.projectId) {
			displayNotificationMessage(info, "Loading Cached Comment Project",
					"Found cached project ID", noiseLevel);
			projectSelectorDiv.append("<option id=\"" + value.projectId
					+ "\" selected=true>" + value.name + "</option>");
...
function getProjectIDforLocation(locationValue) {
	...
	if (projectId === null) {
		displayNotificationMessage(error, "Error getting project ID",
				"Project ID cannot be determined", noisy);
	}
	return projectId;
}
/**
